...
const AddEmployeePopup: React.FC<AddUserPopupProps> = ({ onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const newUser: User = { name, title, email, role };
    onSubmit(newUser);
    onClose();
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center z-99999 bg-black bg-opacity-80 px-4 py-5">
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          {['name', 'title', 'email', 'role'].map((field) => (
            <div className="mb-5" key={field}>
              ...
              <input
                ...
                name={field}
                value={field === 'name' ? name : field === 'title' ? title : field === 'email' ? email : role}
                onChange={(e) => {

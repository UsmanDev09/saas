...
const TaskKanban: React.FC = () => {
  ...
  const handleEditTask = (taskId: string) => {
    ...
    if (taskToEdit) {
      setSelectedTask({
        ...taskToEdit,
        column: taskToEdit.column as 'To-Do' | 'In-Progress' | 'Completed',
      });
      setPopupOpen(true);
    }
  };
        ...
        <div className="mt-9 grid grid-cols-1 gap-7.5 sm:grid-cols-2 xl:grid-cols-3">
          <div className="swim-lane flex flex-col gap-5.5">
            <h4 className="text-xl font-semibold text-black dark:text-white">
              To Do&apos;s ({Tasks.filter((task) => task.column === 'To-Do').length})
            </h4>
            {renderTasks('To-Do')}
          </div>
          ...
          <div className="swim-lane flex flex-col gap-5.5">
            <h4 className="text-xl font-semibold text-black dark:text-white">
              In Progress ({Tasks.filter((task) => task.column === 'In-Progress').length})
            </h4>
          ...
          <div className="swim-lane flex flex-col gap-5.5">
            <h4 className="text-xl font-semibold text-black dark:text-white">
              Completed ({Tasks.filter((task) => task.column === 'Completed').length})
            </h4>

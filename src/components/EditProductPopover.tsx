  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-99999 bg-black bg-opacity-80 px-4 py-5">
      ...
      >
        ...
        <h3 className="text-lg font-semibold mb-4 text-black dark:text-white">Edit Product</h3>
        <form onSubmit={handleSubmit}>
          {['name', 'category', 'price', 'sold', 'profit'].map((field) => (
            <div className="mb-5" key={field}>
              <label className="block mb-2.5 font-medium text-black dark:text-white capitalize">
                {field}
              </label>
              <input
                type={field === 'name' || field === 'category' ? 'text' : 'number'}
                name={field}
                value={field === 'price' || field === 'sold' || field === 'profit'
                  ? editedProduct[field as keyof Product].toString()
                  : editedProduct[field as keyof Product]}
